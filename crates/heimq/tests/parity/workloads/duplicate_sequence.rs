//! Parity workload: hand-crafted duplicate idempotent producer sequence.
...
const TOPIC: &str = "parity-duplicate-sequence";

pub struct DuplicateSequence;

#[async_trait]
impl WorkloadDriver for DuplicateSequence {
...
fn run_sync(bootstrap: &str) -> Result<Vec<Observation>> {
    ...
    let duplicate_code = client.produce_sequence(TOPIC, producer_id, producer_epoch, 0)?;
    Ok(vec![Observation {
        workload: "duplicate_sequence",
        step: 0,
