...
pub fn check_initial_offsets(log: &Arc<dyn PartitionLog>) {
    assert_eq!(log.log_start_offset(), 0, "new partition: log_start_offset must be 0");
    assert_eq!(log.high_watermark(), 0, "new partition: high_watermark must be 0");
}
...
pub fn check_append_advances_hwm(log: &Arc<dyn PartitionLog>) {
    ...
    assert!(log.high_watermark() > 0, "high_watermark must advance after append");
}
...
pub fn check_read_after_append(log: &Arc<dyn PartitionLog>) {
    ...
    assert!(!fetched.is_empty(), "read after append must return non-empty bytes");
}
...
pub fn check_truncate_before(log: &Arc<dyn PartitionLog>) {
    ...
    if hwm > 0 {
        log.truncate_before(hwm).expect("truncate_before must not fail");
        assert!(
