...
pub fn check_commit_overwrites(store: &dyn OffsetStore) {
    store
        .commit("suite-grp-ow", "t", 0, 10, 0, None)
        .expect("first commit");
    store
        .commit("suite-grp-ow", "t", 0, 20, 0, None)
...
pub fn check_fetch_all_for_group(store: &dyn OffsetStore) {
    store
        .commit("suite-grp-fa", "t", 0, 1, 0, None)
        .expect("commit p0");
    store
        .commit("suite-grp-fa", "t", 1, 2, 0, None)
        .expect("commit p1");
    let all = store.fetch_all_for_group("suite-grp-fa");
    assert!(
        !all.is_empty(),
        "fetch_all_for_group must return entries after commit"
    );
    assert_eq!(
...
pub fn check_delete_group(store: &dyn OffsetStore) {
    store
        .commit("suite-grp-del", "t", 0, 5, 0, None)
        .expect("commit");
    store.delete_group("suite-grp-del");
    let fetched = store.fetch("suite-grp-del", "t", 0);
    assert!(
        fetched.is_none(),
        "fetch after delete_group must return None"
    );
}
...
pub fn check_delete_offset(store: &dyn OffsetStore) {
    store
        .commit("suite-grp-do", "t", 0, 10, 0, None)
        .expect("commit p0");
    store
        .commit("suite-grp-do", "t", 1, 20, 0, None)
        .expect("commit p1");
    store.delete_offset("suite-grp-do", "t", 0);
    assert!(
        store.fetch("suite-grp-do", "t", 0).is_none(),
        "fetch after delete_offset must return None"
    );
    assert_eq!(
